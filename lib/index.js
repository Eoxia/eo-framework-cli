...
if ('module' === program.generate && program.args[0]) {
	...
	ncp(pathCLI + '/models/module', options.cwd + '/module/' + name, function (err) {
		if (err) {
			return console.error(err);
		}

		UpdateFileContentModule(options.cwd + '/module/' + name + '/action/module.action.php', pluginSlug, name);
		UpdateFileContentModule(options.cwd + '/module/' + name + '/class/module.class.php', pluginSlug, name);
		UpdateFileContentModule(options.cwd + '/module/' + name + '/view/main.view.php', pluginSlug, name);
		UpdateConf(options.cwd + '/module/' + name + '/module.config.json', pluginSlug, name);

		fs.renameSync(options.cwd + '/module/' + name + '/action/module.action.php', options.cwd + '/module/' + name + '/action/' + name + '.action.php' );
		fs.renameSync(options.cwd + '/module/' + name + '/class/module.class.php', options.cwd + '/module/' + name + '/class/' + name + '.class.php' );
		fs.renameSync(options.cwd + '/module/' + name + '/asset/js/module.backend.js', options.cwd + '/module/' + name + '/asset/js/' + name + '.backend.js' );
		fs.renameSync(options.cwd + '/module/' + name + '/module.config.json', options.cwd + '/module/' + name + '/' + name + '.config.json' );
...
function AddModuleInMainConf(mainConfPath, name) {
	...
	fs.writeFileSync(mainConfPath, JSON.stringify(data), 'utf8');
